...
export class DraggableDirective implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() validateDrag: ValidateDrag | undefined
  @Input() dragCursor: string = ''
  @Input() dragActiveClass: string = ''
  @Input() ghostElementAppendTo: HTMLElement | undefined
  @Input() ghostElementTemplate: TemplateRef<any> | undefined
  @Input() touchStartLongPress: { delay: number; delta: number } = { delay: 500, delta: 10 }
  @Input() autoScroll: {
    margin: number | { top?: number; left?: number; right?: number; bottom?: number }
    maxSpeed?: number | { top?: number; left?: number; right?: number; bottom?: number }
